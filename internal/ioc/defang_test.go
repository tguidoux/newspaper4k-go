...
// TestDefang ensures Defang transformations match the expected defanged form.
func TestDefang(t *testing.T) {
	for _, test := range DefangTests {
		if got := test.input.toDefanged(); !reflect.DeepEqual(got, test.want) {
...
func TestFang(t *testing.T) {
	// Ensure defanged -> fanged yields original input (for DefangTests)
	for _, test := range DefangTests {
		t.Run(test.input.IOC, func(t *testing.T) {
			if got := test.want.toFanged(); !reflect.DeepEqual(got, test.input) {
	...
	for _, test := range FangTests {
		t.Run(test.input.IOC, func(t *testing.T) {
			if got := test.input.toFanged(); !reflect.DeepEqual(got, test.want) {
...
func TestIsFanged(t *testing.T) {
	...
	for _, test := range tests {
		if test.input.isFanged() != test.want {
			t.Errorf("Incorrect fanging result for %s", test.input.String())
		}
	}
}
